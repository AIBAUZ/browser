...
Options:
  ...
  --tag vX.Y.Z                    Release tag (default: v<version.txt>)
...
ensure_tag_matches_app_version() {
  local expected_tag="v$APP_VERSION"
  if [[ -z "$TAG" ]]; then
    TAG="$expected_tag"
    log "Using tag from version.txt: $TAG"
    return
  fi
  [[ "$TAG" == "$expected_tag" ]] || die "Tag/version mismatch: tag=$TAG but version.txt=$APP_VERSION (expected $expected_tag)"
}
